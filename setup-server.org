...
apt install build-essential make gcc g++ gdb clang clang-format vim python3 git default-jdk docker.io quota zip unzip valgrind sshpass ripgrep inotify-tools fail2ban zsh;
apt autoremove;
apt clean;
apt autoclean;
apt autopurge;
...
* Clone class-utils via https and copy skel folder's files into the /etc/skel directory
