...
useradd -m -N -g bot -G sudo -s /bin/bash -c "Autograder" -K UID_MIN=1000 -K UID_MAX=1999 -K UMASK=077 autograder;
useradd -m -N -g student -s /bin/bash -c "Test Student" -K UID_MIN=2000 -K UMASK=077 test_08675309;

passwd lawt;
passwd autograder;

* (Optional) Make a swap file
...
Looks like you have to add your private key to root now for refresh-class-utils to work?
...
apt install build-essential make gcc g++ gdb clang clang-format vim \ python3 python3-pip python3-venv git default-jdk quota zip unzip valgrind \ sshpass ripgrep inotify-tools fail2ban sl zsh cowsay lolcat emacs fzf ncdu;
apt autoremove;
apt clean;
apt autoclean;
apt autopurge;
...
~autograder:
python3 -m venv data-8-assignments-venv
data-8-assignments-venv/bin/pip install otter-grader==6.1.3
data-8-assignments-venv/bin/pip install datascience==0.17.6
data-8-assignments-venv/bin/pip install ipykernel==6.30.0
data-8-assignments-venv/bin/pip install setuptools<82.0.0
...
ignoreip = ... 209.129.240.0/22 205.155.0.0/16
...
(don't actually do this--it breaks nvim)
sudo chmod 751 /home
(usually it's 755)
...
if this ever stops working, or if you want to use ext4 quotas without quota files (but you have to use a recovery console because the drive needs to be unmapped):
https://askubuntu.com/a/1492149
...
* Set up docker (no longer necessary)
...
python3 -m venv ~/data-8-assignments-venv
source ~/data-8-assignments-venv/bin/activate
pip install otter-grader==6.1.3
pip install datascience==0.17.6
pip install ipykernel==6.30.0
...
* Install DATA 8 submission (data_8_submit on github)

clone into autograder's home folder
...
in a tmux split pane window:
node app.js
sudo caddy reverse-proxy --from server.lawtonsclass.com --to localhost:3000

if caddy says port 80 is in use:
sudo caddy stop

* Things to run in tmux on the servers

** autograder@server:~/data_8_submit
...
** lawt@www:~/class_website

(just initially)
sudo service caddy stop

CLASS_WEBSITE_LOG=info ./target/release/class_website_v3
sudo caddy reverse-proxy --from www.lawtonsclass.com --to :8000
