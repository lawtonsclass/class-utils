...
ssh -i ~autograder/.ssh/id_rsa lawt@www.lawtonsclass.com "mkdir -p ~/class_website/grades/$user" || \
  echo "Error connecting to the class website. Please tell Lawton about this." >> "$user_home_folder"/.autograder/log
scp -i ~autograder/.ssh/id_rsa ~autograder/grades/$resultfilename lawt@www.lawtonsclass.com:~/class_website/grades/$user || \
